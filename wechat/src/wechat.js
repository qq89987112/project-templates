...
const crypto = require('crypto'), //引入加密模块
  ...
  cache = require('./cache.json'), //引入本地存储的 access_token
...
var WeChat = function (config) {
  ...
  this.getAccessToken().then((data) => {
    ...
    let menuData = JSON.stringify(menus);
    this.requestPost(url, menuData).then(function (data) {
      //讲结果打印
      console.log("创建菜单成功!");
    });
  }).catch((data) => {
    console.log("创建菜单失败!", data);
  });
}
...
WeChat.prototype.getAccessToken = function () {
    return this.getCache("access_token",util.format(this.apiURL.accessTokenApi, this.apiDomain, this.appID, this.appScrect));
};

WeChat.prototype.getCache = function (key,url) {
  return new Promise((resolve, reject) =>{
    //获取当前时间
    var currentTime = new Date().getTime();
    //判断 本地存储的 access_token 是否有效
    let value =cache[key];
    if (value.value === "" || value.expires_time < currentTime) {
      return this._getCacheFromUrl(url,key);
    } else {
...
WeChat.prototype._getCacheFromUrl = function (url,key) {
  let obj =cache[key];
  return this.requestGet(url).then(function (data) {
    var result = JSON.parse(data);
    if (!result.errcode || result.errcode === 0) {
      obj.value = result[key];
      obj.expires_time = new Date().getTime() + (parseInt(result.expires_in) - 200) * 1000;
      //更新本地存储的
      fs.writeFile(path.join(__dirname,'./cache.json'), JSON.stringify(cache));
...
WeChat.prototype.sign = function (url) {
  return this.getAccessToken().then((data) => {
    return this.getCache("ticket",util.format(this.apiURL.jsapi_ticket, data))
  }).then((data)=>{
    let timestamp = new Date().getTime(),
     str = `jsapi_ticket=${data}&noncestr=Wm3WZYTPz0wzccnW&timestamp=${timestamp}&url=${url}`;
    console.log(str);
    const hashCode = crypto.createHash('sha1'); //创建加密类型
